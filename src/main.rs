...
use hyprswitch::{DaemonInfo, DRY, handle, Info};
...
async fn run_daemon(
    ...
    if !daemon::daemon_running().await {
        ...
        let data = handle::collect_data(info)
            .await
        ...
        if do_initial_execute {
            if switch_on_close {
                ...
                let (next_client, new_index) =
                    handle::find_next(info, lock.1.enabled_clients.clone(), lock.1.selected_index)
                        .with_context(|| {
        ...
        std::thread::spawn(move || {
            let switch = move |next_client: Client, latest_data: Share| async move {
                handle::switch_async(&next_client, *DRY.get().expect("DRY not set"))
                    .await
                    .with_context(|| {
        ...
        let exec = move |daemon_info: DaemonInfo, latest_data: Share| async move {
            ...
            let (next_client, new_index) =
                handle::find_next(new_info, ld.1.enabled_clients.clone(), ld.1.selected_index)
