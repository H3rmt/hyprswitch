...
lazy_static! {
    static ref SIZE_FACTOR: i16 =
        option_env!("SIZE_FACTOR").map_or(7, |s| s.parse().expect("Failed to parse SIZE_FACTOR"));
    static ref ICON_SIZE: i32 =
        option_env!("ICON_SIZE").map_or(128, |s| s.parse().expect("Failed to parse ICON_SIZE"));
    static ref ICON_SCALE: i32 =
        option_env!("ICON_SCALE").map_or(1, |s| s.parse().expect("Failed to parse ICON_SCALE"));
    static ref NEXT_INDEX_MAX: i32 = option_env!("NEXT_INDEX_MAX")
        .map_or(5, |s| s.parse().expect("Failed to parse NEXT_INDEX_MAX"));
    static ref EXIT_ON_CLICK: bool = option_env!("EXIT_ON_CLICK")
        .map_or(true, |s| s.parse().expect("Failed to parse EXIT_ON_CLICK"));
    static ref WORKSPACE_GAP: usize = option_env!("WORKSPACE_GAP")
        .map_or(15, |s| s.parse().expect("Failed to parse WORKSPACE_GAP"));
}
...
fn update<F: Future<Output=anyhow::Result<()>> + Send + 'static>(
    ...
    let (monitor_id, _monitor_data) = data
        .1
        .monitor_data
        .iter()
        .find(|(_, v)| v.connector == connector)
    ...
    for (id, workspace) in workspaces.iter().enumerate() {
        ...
        for client in clients {
            let client_active = data
                .1
                .active
                .as_ref()
                .map_or(false, |active| active.address == client.address);
            let index = data
                .1
                .enabled_clients
                .iter()
            ...
            let frame = client_ui(
                ...
                data.1
                    .enabled_clients
                    .iter()
                    .any(|c| c.address == client.address),
            );
            ...
            tokio::runtime::Runtime::new()
                ...
                .block_on(async {
                    ...
                    let empty = {
                        ...
                        !d.1.clients
                            .iter()
                            .any(|client| client.monitor == *monitor_id)
                    };
                    if !empty {
                        let data = data.clone();
